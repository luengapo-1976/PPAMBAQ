...
type Estado = 'pendientes' | 'aprobados';
...
export class CasosPorValidar {
  ...
  protected readonly rowsTurnos = computed(() =>
    this.estado() === 'pendientes' ? this.turnosPendientes() : this.turnosAprobados(),
  );
  ...
  protected readonly dialogTitulo = computed(() =>
    this.dialogPaso() === 'justificacion' ? 'Aprobar solicitud' : 'Detalle del caso',
  );

  protected readonly dialogMuestraPrimario = computed(
    () => this.dialogPaso() === 'justificacion' || (this.dialogPaso() === 'detalle' && this.estado() === 'pendientes'),
  );

  protected readonly dialogPrimaryLabel = computed(() =>
    this.dialogPaso() === 'justificacion' ? (this.aprobando() ? 'Enviando…' : 'Enviar aprobación') : 'Aprobar solicitud',
  );

  protected readonly dialogPrimaryDisabled = computed(
    () => this.dialogPaso() === 'justificacion' && (!this.justificacionAprobacion().trim() || this.aprobando()),
  );

  protected readonly dialogSecondaryLabel = computed(() => (this.dialogPaso() === 'justificacion' ? 'Cancelar' : 'Cerrar'));
  ...
  protected nombreCompletoRetiro(retiro: RetiroResumen): string {
    const nombre = [retiro.primer_nombre, retiro.segundo_nombre, retiro.primer_apellido, retiro.segundo_apellido]
  ...
  protected onDialogPrimaryAction(): void {
    if (this.dialogPaso() === 'justificacion') {
      this.onEnviarAprobacion();
    } else {
      this.onAbrirAprobacion();
    }
  }

  protected onDialogSecondaryAction(): void {
    if (this.dialogPaso() === 'justificacion') {
  ...
  protected onAbrirAprobacion(): void {
  ...
  protected onEnviarAprobacion(): void {
    ...
    if (this.tipo() === 'turnos') {
      ...
      this.casosService.aprobarTurno(turno.id, justificacion).subscribe({
        next: () => {
          this.aprobando.set(false);
          this.snackbar.success('Solicitud aprobada correctamente.');
          this.onCerrarDialog();
          this.cargarTurnos();
        },
        error: (err: ApiError) => {
          this.aprobando.set(false);
          this.snackbar.error(err?.message ?? 'No se pudo aprobar la solicitud. Intenta nuevamente.');
        },
      });
      return;
    }
    ...
    this.casosService.aprobarRetiro(retiro.id, justificacion).subscribe({
      ...
      error: (err: ApiError) => {
        this.aprobando.set(false);
        this.snackbar.error(err?.message ?? 'No se pudo aprobar la solicitud de baja. Intenta nuevamente.');
      },
    });
  }
  ...
  private cargarTodo(): void {
    ...
    forkJoin({
      turnosPendientes: this.casosService.turnosPendientes().pipe(catchError(() => of<TurnoValidacionResumen[]>([]))),
      turnosAprobados: this.casosService.turnosAprobados().pipe(catchError(() => of<TurnoValidacionResumen[]>([]))),
      retirosPendientes: this.casosService.retirosPendientes().pipe(catchError(() => of<RetiroResumen[]>([]))),
      retirosAprobados: this.casosService.retirosAprobados().pipe(catchError(() => of<RetiroResumen[]>([]))),
    }).subscribe((resultado) => {
